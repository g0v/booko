...
const DocCard: React.FC<DocCardProps> = ({ doc }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden flex flex-col md:flex-row h-full group hover:shadow-xl transition-all duration-500">
      ...
      <div className="md:w-[280px] shrink-0 relative overflow-hidden bg-stone-900 aspect-[2/3] md:aspect-[2/3]">
        ...
        >
          {imageError ? (
            <div className="w-full h-full flex flex-col items-center justify-center text-stone-600 bg-stone-100">
      ...
      {/* Content Section */}
      <div className="p-8 flex flex-col flex-grow justify-center bg-white">
        <div className="flex flex-wrap items-center gap-4 text-stone-400 text-[11px] font-bold mb-4 uppercase tracking-wider">
          <div className="flex items-center space-x-1.5">
            <Calendar size={13} className="text-rose-700/50" />
            <span>首映年份：{doc.year}</span>
          </div>
          <div className="flex items-center space-x-1.5 border-l border-stone-200 pl-4">
            <User size={13} className="text-rose-700/50" />
            <span>導演：{doc.director}</span>
          </div>
        </div>
        
        <h3 className="text-3xl font-black text-stone-900 serif mb-4 group-hover:text-rose-800 transition-colors leading-tight">
          《{doc.title}》
        </h3>
        
        <p className="text-stone-600 text-sm leading-relaxed mb-10 max-w-xl serif italic border-l-4 border-rose-100 pl-6">
          {doc.description}
        </p>
        
        <div className="mt-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pt-6 border-t border-stone-100">
          <a 
            href={searchUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="group/btn relative inline-flex items-center bg-stone-900 hover:bg-rose-800 text-white px-8 py-3.5 rounded-full text-sm font-bold transition-all duration-300 shadow-lg hover:shadow-rose-200 hover:-translate-y-0.5 overflow-hidden"
          >
          ...
          <div className="flex flex-col items-end">
            <span className="text-[10px] text-stone-400 font-bold uppercase tracking-widest mb-1">Source: YouTube Search</span>
            <span className="text-[10px] text-rose-700/60 font-medium">點擊按鈕或圖片探索更多紀錄片資訊</span>
          </div>
        </div>
      </div>
    </div>
