import { BookOpen, Film, History, Palette, Pencil } from 'lucide-react';
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fbfaf8] dark:bg-[#1e1e1e] transition-colors duration-300">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 sm:h-20">
            <Link to="/" className="flex items-center space-x-3 group">
              <History
                className="text-rose-700 dark:text-rose-400 transform group-hover:rotate-12 transition-transform"
                size={28}
              />
      ...
      <main className="flex-grow max-w-[1400px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 sm:py-12">{children}</main>
      ...
      <footer className="bg-stone-900 dark:bg-stone-950 text-stone-400 py-16 transition-colors duration-300">
        <div className="max-w-[1400px] mx-auto px-4 text-center">
          ...
          <p className="serif text-stone-200 text-xl mb-4 max-w-2xl mx-auto leading-relaxed">
            自由，是每一代人的接力賽。
          </p>
